...
def test_profiles_command_lists_catalog():
    ...
    assert "eu-cra-annex-i" in result.output
...
def test_validate_json_multi_profile_independent():
    result = CliRunner().invoke(cli, [
        ...
        "--profile", "ntia-min-elements", "--profile", "eu-cra-annex-i"])
    payload = json.loads(result.output)
    assert len(payload["results"]) == 2
    ids = {r["profile"] for r in payload["results"]}
    assert ids == {"ntia-min-elements", "eu-cra-annex-i"}
...
def test_validate_sarif_one_run_per_profile():
    ...
    assert names == {"ossbomer:ntia-min-elements", "ossbomer:eu-cra-annex-i"}
